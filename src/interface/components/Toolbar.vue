<template>
  ...
  >
    <v-menu v-if="formatTools.length" show-arrow placement="bottom-start" :fullHeight="true">
...
<script setup lang="ts">
  ...
  function useFloatingMenu() {
    const padding =
      parseInt(
        getComputedStyle(document.body)?.getPropertyValue('--theme--popover--menu--border-radius')?.replace('px', ''),
        10
      ) ?? 6;
