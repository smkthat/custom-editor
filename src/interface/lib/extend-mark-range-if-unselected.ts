...
export function extendMarkRangeIfUnselected(editor: Editor, mark: string) {
  ...
  const siblingCharIsSpace =
    doc.textBetween(from, from + 1, undefined, ' ') === ' ' || doc.textBetween(from - 1, from, undefined, ' ') === ' ';

  const chain = editor.chain().focus();

  return textIsSelected || isAtEndOfLine || siblingCharIsSpace ? chain : chain.extendMarkRange(mark);
}
