...
<template>
  <node-view-wrapper :class="inline ? 'relation-inline-block' : 'relation-block'">
    ...
    >
      ...
      <span v-if="!inline" class="collection">{{ getCollectionName(element) }}:</span>
      <render-template
      ...
      <v-icon class="clear-icon" name="delete" @click.stop="deleteNode" :small="inline" />
    </div>

    <template v-else>
      <v-notice v-if="!inline" type="warning">
        <span>{{ t('related_item_missing') }}</span>
        <div class="spacer"></div>
        <v-icon class="clear-icon" name="delete" @click.stop="deleteNode" />
      </v-notice>

      <div v-else class="inline-warning">
        <v-icon name="warning" small />
        <span>{{ t('related_item_missing') }}</span>
        <v-icon class="clear-icon" name="delete" @click.stop="deleteNode" small />
      </div>
    </template>
  </node-view-wrapper>
</template>
...
<style scoped>
  ...
  .v-list-item.selected {
    --v-list-item-border-color: var(
      --theme--form--field--input--border-color-hover,
      var(--v-list-item-border-color-hover)
    );
  }
