<template>
  <v-menu show-arrow placement="bottom-start" :fullHeight="true">
    <template #activator="{ toggle }">
      <ToolButton :title :disabled :icon="currentAlignment.icon" :action="toggle" />
    </template>
    <v-list>
      <v-list-item
        clickable
...
<script setup lang="ts">
  ...
  const currentAlignment = computed(
    () =>
      alignmentOptions.find((option) => props.editor.isActive({ textAlign: option.alignment })) ?? alignmentOptions[0]
  );
</script>
