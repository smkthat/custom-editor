...
export default defineTool({
  ...
  extension: [
    (selection) => {
      const types = selection?.filter((extension) => ['paragraph', 'codeBlock'].includes(extension));

      const anyHeading = selection?.some((extension) => ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(extension));
