<template>
  <v-card class="card">
    ...
    <v-card-actions>
      <v-button @click="$emit('closeDialog')" secondary>{{ t('cancel') }}</v-button>
      <v-button @click="onRemove" :disabled="oldLinkIsEmpty" danger>{{ t('remove') }}</v-button>
      <v-button @click="onAdd" :disabled="linkIsEmpty">{{ t('save') }}</v-button>
    </v-card-actions>
  </v-card>
</template>
