  ...
  >
    ...
    <v-list>
      <v-list-item v-if="selectedRelationMark?.attrs?.noRelatedItem">
        <v-list-item-icon>
          <v-icon name="warning" class="warning" />
        </v-list-item-icon>
        <v-list-item-content>{{ t('related_item_missing') }}</v-list-item-content>
      </v-list-item>

      <v-list-item v-else clickable @click="edit">
        <v-list-item-icon><v-icon name="edit" /></v-list-item-icon>
        <v-list-item-content><v-text-overflow :text="t('edit')" /></v-list-item-content>
      </v-list-item>

      <v-list-item clickable @click="editor.chain().focus().unsetRelationMark().run()">
...
<script setup lang="ts">
  ...
  const selectedRelationMark = computed(() =>
