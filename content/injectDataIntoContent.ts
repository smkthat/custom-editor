 ...
 */
export default (data: EditorNodes[], content: JSONContent, primaryKeyField = 'id', itemField = 'item') => {
  ...
  function toContentWithInjectedData(content: JSONContent) {
    ...
    if (content.type && relationBlockNodes.indexOf(content.type) >= 0 && content.attrs?.id) {
